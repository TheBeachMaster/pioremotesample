				...
				if (type == MQTTPUBLISH)
				{
					if (callback)
					{
						...
						if ((buffer_[0] & 0x06) == MQTTQOS1)
						{
							...
							//calllback_arbitartor(topic, payload, len - llen - 3 - tl - 2);
						...
						else
						{
							...
							// calllback_arbitartor(topic, payload, len - llen - 3 - tl);
...
boolean AfricasTalkingCloudClient::publish(const char* topic, const char* payload, boolean retained)
{
	return publish(topic, reinterpret_cast<const uint8_t*>(payload), strlen(payload), 0, retained);
}
...
boolean AfricasTalkingCloudClient::publish_progmem(const char* topic, const uint8_t* payload, boolean retained)
{
	return publish_progmem(topic, payload, strlen(payload), retained);
}
